...
public class UpdateTourCommandValidator : AbstractValidator<UpdateTourCommand>
{
    ...
    public UpdateTourCommandValidator(IApplicationDbContext context)
    {
        _context = context;
    ...
    private async Task<bool> TourTypesExist(List<int> tourTypeIds, CancellationToken cancellationToken)
        => await _context.TourTypes.CountAsync(tt => tourTypeIds.Contains(tt.Id), cancellationToken) == tourTypeIds.Count;
}
